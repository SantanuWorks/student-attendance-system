<?php
	session_start();
	if( !isset($_SESSION['ADMIN']) ) header('Location: index.php'); 
	if(isset($_POST['cancel'])) header('location: adminform.php');
?>

<?php
	$servername = "localhost:3307";
	$username = "root";
	$password = "";
	$db = 'student' ;
	$conn = mysqli_connect($servername, $username, $password, $db);
	if(!$conn) header('location: adminform.php') ;
	$qry = 'select * from studinfo;' ;
	$res = mysqli_query($conn, $qry) ;
	$stuid = mysqli_num_rows($res) + 1 ;
?>

<!DOCTYPE html>
<html>
<head>
	<title>Add Student Information</title>
	<link rel="stylesheet" type="text/css" href="main.css">
</head>
<body>
	<h1 >Student Information</h1>
	<div class="main_box" >
		<form action="studentinfo.php" method="post">
			<label >Student Id</label><br>
			<input class="margin_10_10" type="text" name="stuid" disabled value="<?php echo $stuid ; ?>"><br>
			<label>Student Name</label><br>
			<input class="margin_10_10" type="text" name="name"><br>
			<label>Course</label><br>
			<input class="margin_10_10" type="text" name="course" disabled value="MCA"><br>
			</select><br>
			<label>Semester</label><br>
			<input class="margin_10_10" type="number" min="1" max="4" name="sem"><br>
			</select><br>
			<input class="margin_10_10" type="submit" name="save" value="Save">
			<input class="margin_10_10" type="reset" name="clear" value="Clear">
			<input class="margin_10_10" type="submit" name="cancel" value="Cancel">
		</form>
	</div>
</body>
</html>

<?php
if(isset($_POST['save'])){
	$name = $_POST['name'] ;
	$crs = 'MCA' ;
	$sem = $_POST['sem'] ;
	$qry = 'insert into studinfo values ( "'.$stuid.'", "'.$name.'", "'.$crs.'", "'.$sem.'" );' ;
	$res = mysqli_query($conn, $qry) ;
	if($res) header("location: adminform.php") ;
	else header('location: studentinfo.php') ;
}
?>